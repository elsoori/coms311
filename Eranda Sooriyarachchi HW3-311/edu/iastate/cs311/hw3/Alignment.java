...
// traceback method for optimum alignment
  ...
  while (i <= M && j <= N) {
      ...
      //insertion in pair
      else if (i == M || ((Smat[i][j]) == (Smat[i][j + 1] + r))) {
          top[alignlen] = ' ';
          mid[alignlen] = '-';
          bot[alignlen] = B[j + 1];
          j = j + 1;
      ...
      // substituition or no mutation of pair
      else {
          top[alignlen] = A[i + 1];
          if (A[i + 1] == B[j + 1]) {
              mid[alignlen] = '|';
